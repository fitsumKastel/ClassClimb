...
function executeGet(store, sql, params) {
    ...
    if (sql.startsWith('SELECT 1 AS OK FROM SUBSCRIPTIONS WHERE CLASS_ID = ? AND TELEGRAM_ID = ?')) {
        return store.hasSubscriptionForTelegram(params[0], params[1]) ? { ok: 1 } : undefined;
    }
    if (

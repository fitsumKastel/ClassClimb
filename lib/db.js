...
(function migrateClassPdfColumns() {
    try {
        ...
        rawDb.exec('UPDATE classes SET class_pdf_rev = 0 WHERE class_pdf_rev IS NULL');
        rawDb.exec('UPDATE classes SET pdf_follow_active = 0 WHERE pdf_follow_active IS NULL');
        rawDb.exec('UPDATE classes SET pdf_follow_page = 1 WHERE pdf_follow_page IS NULL');
    } catch {
        /* ignore */
    }
})();

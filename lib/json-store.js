...
class JsonStore {
    ...
    insertClass({ teacher_id, class_name, school_name, view_id }) {
        const vid = String(view_id || '').trim();
        if (!vid) {
            throw new Error('view_id is required');
        }
        const existing = this._readCollection('classes');
        if (existing.some((r) => String(r.view_id) === vid)) {
            const err = new Error('UNIQUE constraint failed: classes.view_id');
    ...
    countSubscriptionsByTelegram(telegramId) {
        return this._readCollection('subscriptions').filter(
    ...
    listBoardsForTelegram(telegramId) {
        ...
        for (const s of subs) {
            ...
            if (!c) continue;
            // Classes you teach are managed on /teacher, not student boards.
            if (c.teacher_id != null && String(c.teacher_id) === viewer) continue;
            out.push({
                id: c.id,
                view_id: c.view_id,
    ...
    hasSubscriptionForTelegram(classId, telegramId) {
    ...
    insertSubscriptionIgnore({ link_id, telegram_id, class_id }) {
        ...
        const dupTelegram = rows.some(
            (r) =>
                Number(r.class_id) === Number(class_id) &&
                String(r.telegram_id) === String(telegram_id)
        );
        if (dupLink || dupTelegram) {
